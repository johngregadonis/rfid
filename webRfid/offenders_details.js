...
if (operator) {
  ...
  async function fetchBalanceChangeLogs() {
    try {
      const response = await fetch(`http://localhost:2000/get-balance-change?bodyNumber=${encodeURIComponent(operator.bodyNumber)}`);
      if (!response.ok) {
...
async function addBalanceToDB() {
  ...
  try {
    ...
    const fineResponse = await fetch('http://localhost:2000/save-fine-payment', {
      method: 'POST',
      headers: {
    ...
    if (balanceAmount > 0) {
      const balanceResponse = await fetch('http://localhost:2000/update-balance', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
