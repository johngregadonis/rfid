...
async function fetchVehicleOperators() {
  try {
      const response = await fetch('http://192.168.38.88:2000/get-vehicle-operators');
...
async function addBalanceToDB() {
  ...
  try {
      const response = await fetch('http://192.168.38.88:2000/update-balance', {
          method: 'POST',
          headers: {
...
try {
  const response = await fetch('http://localhost:2000/rfid', {
    method: 'POST',
